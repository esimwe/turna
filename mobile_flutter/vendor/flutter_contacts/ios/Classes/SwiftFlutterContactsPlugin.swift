...
public class SwiftFlutterContactsPlugin: NSObject, FlutterPlugin, FlutterStreamHandler, CNContactViewControllerDelegate, CNContactPickerDelegate {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let instance = SwiftFlutterContactsPlugin(registrar.viewController)
    ...
    private static func fallbackRootViewController() -> UIViewController? {
        if #available(iOS 13.0, *) {
            let windowScenes = UIApplication.shared.connectedScenes
                .compactMap { $0 as? UIWindowScene }
                .sorted { lhs, rhs in
                    let lhsScore = lhs.activationState == .foregroundActive ? 2 : (lhs.activationState == .foregroundInactive ? 1 : 0)
                    let rhsScore = rhs.activationState == .foregroundActive ? 2 : (rhs.activationState == .foregroundInactive ? 1 : 0)
                    return lhsScore > rhsScore
                }
            for scene in windowScenes {
                if let controller = scene.windows.first(where: \.isKeyWindow)?.rootViewController {
                    return controller
                }
                if let controller = scene.windows.first(where: { !$0.isHidden })?.rootViewController {
                    return controller
                }
            }
        }
    ...
    private func presenterViewController() -> UIViewController? {
        topViewController(from: rootViewController) ?? topViewController(from: Self.fallbackRootViewController())
    }
            ...
            DispatchQueue.main.async {
                guard let presenter = self.presenterViewController() else {
                    result(FlutterError(
                        ...
                        message: "No root view controller available for presenting contacts UI.",
                        details: nil
                    ))
                    return
                }
