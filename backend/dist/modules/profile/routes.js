...
import { assertObjectExists, createAvatarUploadUrl, deleteObject, getObjectBytes, isAvatarKeyOwnedByUser, isStorageConfigured } from "../../lib/storage.js";
...
const nullableTrimmedString = (maxLength) => z.preprocess((value) => {
...
function toProfileDto(req, user) {
    return {
        id: user.id,
...
function toPublicProfileDto(req, user) {
    return {
        id: user.id,
        displayName: user.displayName,
        username: user.username,
...
profileRouter.get("/username-availability", requireAuth, async (req, res) => {
    ...
    const available = currentUsername === username ||
...
profileRouter.put("/onboarding", requireAuth, async (req, res) => {
    ...
    const usernameAvailable = await ensureUsernameAvailable(parsed.data.username, req.authUserId);
...
profileRouter.post("/avatar/complete", requireAuth, async (req, res) => {
    ...
    if (previousUser?.avatarUrl && previousUser.avatarUrl !== parsed.data.objectKey) {
        deleteObject(previousUser.avatarUrl).catch((error) => {
