...
import { cancelCallReconnectGrace, cancelCallTimeout, scheduleCallReconnectGrace, scheduleCallTimeout } from "./call.timeout.js";
export const callRouter = Router();
const liveKitWebhookReceiver = env.LIVEKIT_API_KEY && env.LIVEKIT_API_SECRET
...
function withAvatarUrl(origin, user) {
    return {
        id: user.id,
...
function serializeCallForViewer(origin, call, viewerId) {
...
function serializeHistoryItem(origin, item) {
    return {
        id: item.id,
        type: item.type,
        status: item.status,
...
async function emitResolvedCall(origin, call, eventName, pushReason = call.status) {
...
function respondCallError(res, error) {
...
callRouter.post("/:callId/decline", requireAuth, async (req, res) => {
    ...
    try {
        ...
        void emitResolvedCall(origin, call, "call:declined", "declined").catch((error) => {
...
callRouter.post("/:callId/end", requireAuth, async (req, res) => {
    ...
    try {
        ...
        void emitResolvedCall(origin, call, "call:ended", call.status).catch((error) => {
