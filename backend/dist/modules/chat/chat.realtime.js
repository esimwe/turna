...
const socketIdsBySessionId = new Map();
...
export function registerUserSocket(userId, socketId, sessionId) {
    ...
    if (sessionId) {
        const sessionSocketIds = socketIdsBySessionId.get(sessionId) ?? new Set();
        sessionSocketIds.add(socketId);
        socketIdsBySessionId.set(sessionId, sessionSocketIds);
    }
    return !wasOnline;
}
export function unregisterUserSocket(userId, socketId, sessionId) {
...
export async function emitSessionRevoked(sessionId, reason) {
    if (!chatIo)
        return;
