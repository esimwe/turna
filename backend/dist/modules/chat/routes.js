...
import { createChatAttachmentUploadUrl, isChatAttachmentKeyOwnedByUser, isStorageConfigured } from "../../lib/storage.js";
...
import { emitChatMessage, emitChatStatus, emitInboxUpdate, getSocketsInUserRoom } from "./chat.realtime.js";
...
const editMessageSchema = z.object({
    text: z
        .preprocess((value) => (typeof value === "string" ? value.trim() : value), z.string().min(1).max(4000))
});
...
chatRouter.get("/", requireAuth, async (req, res) => {
    ...
    res.json({
        data: chats.map((chat) => ({
            ...
            avatarUrl: chat.peerId && chat.peerAvatarKey && chat.peerUpdatedAt
...
chatRouter.post("/:chatId/mute", requireAuth, async (req, res) => {
    ...
    try {
        const muted = await chatService.setChatMuted(parsedParams.data.chatId, req.authUserId, parsedBody.data.muted);
        res.json({ data: { muted } });
    }
    catch (error) {
...
chatRouter.post("/:chatId/archive", requireAuth, async (req, res) => {
    ...
    try {
        const archived = await chatService.setChatArchived(parsedParams.data.chatId, req.authUserId, parsedBody.data.archived);
        emitInboxUpdate([req.authUserId]);
...
chatRouter.post("/:chatId/folder", requireAuth, async (req, res) => {
    ...
    try {
        const folder = await chatService.setChatFolder(parsedParams.data.chatId, req.authUserId, parsedBody.data.folderId);
        emitInboxUpdate([req.authUserId]);
...
chatRouter.post("/:chatId/block", requireAuth, async (req, res) => {
    ...
    try {
        const blocked = await chatService.setDirectChatBlocked(parsedParams.data.chatId, req.authUserId, parsedBody.data.blocked);
    ...
    try {
        ...
        if (recipientIds.length > 0) {
            chatService
                ...
                .then((senderDisplayName) => sendChatMessagePush({
                message,
                senderDisplayName,
...
chatRouter.put("/messages/:messageId", requireAuth, async (req, res) => {
    ...
    const userId = req.authUserId;
    try {
        const message = await chatService.editMessage(parsedParams.data.messageId, userId, parsedBody.data.text);
