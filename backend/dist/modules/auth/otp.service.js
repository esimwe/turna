...
import { buildDefaultDisplayName, formatE164Phone, normalizeCountryIso, normalizeDialCode, normalizeE164Phone, normalizeNationalNumber } from "./phone.js";
import { assertOtpRequestAllowed, getOtpCooldownSeconds, setOtpCooldown } from "./rate-limit.js";
const prismaUser = prisma.user;
const prismaOtpCode = prisma.otpCode;
...
export class OtpService {
    ...
    async getCountryPolicy(countryIso) {
    ...
    async ensureLoginOtpAllowed(input) {
    ...
    async ensurePhoneChangeOtpAllowed(countryIso) {
        ...
        if (!countryPolicy.isServiceEnabled ||
            !countryPolicy.isOtpEnabled ||
            !countryPolicy.isPhoneChangeEnabled) {
            throw new Error("phone_change_temporarily_unavailable");
        }
    }
    async consumeExistingOtps(phone, purpose) {
    ...
    async verifyLoginOtp(input, req) {
    ...
    async requestPhoneChangeOtp(input) {
        const normalized = this.normalizePhoneRequest(input);
