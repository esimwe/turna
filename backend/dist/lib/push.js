...
let apnsBearerCache = null;
let apnsVoipKeyErrorLogged = false;
const prismaDeviceToken = prisma.deviceToken;
const prismaChatMember = prisma.chatMember;
function hasFirebaseCredentials() {
    return Boolean(env.FIREBASE_SERVICE_ACCOUNT_JSON ||
        (env.FIREBASE_PROJECT_ID && env.FIREBASE_CLIENT_EMAIL && env.FIREBASE_PRIVATE_KEY));
}
function hasApnsVoipCredentials() {
    return Boolean(env.APNS_TEAM_ID &&
        env.APNS_KEY_ID &&
        env.APNS_BUNDLE_ID &&
...
function getApnsVoipPrivateKey() {
...
function buildPushBody(message) {
...
function createApnsBearerToken() {
    if (!hasApnsVoipCredentials())
        return null;
...
async function markInactiveDeviceTokens(deviceIds) {
...
function getInvalidFcmTokenIds(response, devices) {
...
function summarizeFcmFailures(response, devices) {
    return response.responses
        .map((result, index) => {
        if (result.success)
            return null;
...
async function findActiveDevices(userIds) {
    if (userIds.length === 0)
        return [];
...
async function countUnreadMessagesForUser(userId) {
    ...
    for (const membership of memberships) {
        const cutoff = membership.hiddenAt && membership.clearedAt
...
async function sendApnsVoipPayload(devices, payload) {
...
export async function sendChatMessagePush(params) {
    ...
    const devices = (await findActiveDevices(eligibleRecipientUserIds)).filter((device) => device.tokenKind === "STANDARD");
    if (devices.length === 0) {
        logInfo("chat push skipped", {
...
export async function sendIncomingCallPush(params) {
    ...
    if (devices.length === 0)
        return;
    const androidStandardDevices = devices.filter((device) => device.platform === PushPlatform.ANDROID && device.tokenKind === "STANDARD");
    const iosVoipDevices = devices.filter((device) => device.platform === PushPlatform.IOS && device.tokenKind === "VOIP");
...
export async function sendCallEndedPush(params) {
    if (params.recipientUserIds.length === 0) {
        return;
    }
    const devices = await findActiveDevices(params.recipientUserIds);
